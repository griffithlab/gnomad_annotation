...
input_parser.add_argument(
    '--add_subpopulations',
    action='store_true',
    help="Provide allele frequencies for all subpopulations. May be used with --add_allele_count option. Subpopulations not currently supported by --cuttoff option."
)

args = input_parser.parse_args()

# Choose the correct gnomAD version
if args.add_subpopulations is True:
    vcf_key = "_".join([args.build, args.gnomad_version, args.gnomad_type,'subpopulations'])
else:
...
vcf_loc = {
    ...
    'GRCH37_2.0.1_exomes_subpopulations': '/gscmnt/gc2602/griffithlab/kcotto/GRCH37_2.0.1_exomes.subpopulations.trie.pickle',
    'GRCH37_2.0.1_genomes_subpopulations': '/gscmnt/gc2602/griffithlab/kcotto/GRCH37_2.0.1_genomes.subpopulations.trie.pickle'
}
...
def tsv_header(gnomad_type, add_allele_count, add_subpopulations):
    if add_allele_count:
        if add_subpopulations:
            tsv = ["AC", "AN", "AF",
            ...
            if gnomad_type == 'exomes':
                tsv.extend(["AC_SAS", "AN_SAS", "AF_SAS"])
        else:
            tsv = ["AC", "AN", "AF"]
    else:
        if add_subpopulations:
            tsv = ["AF", "AF_AFR", "AF_AMR", "AF_ASJ", "AF_EAS",
...
def trie_header(gnomad_type, add_subpopulations):
    if add_subpopulations:
        if gnomad_type == 'exomes':
            trie = ["AF", "AC", "AN",
...
def annotate(**kwargs):
    ...
    with open(mutation_filename, "r") as mgi_tsv, open(file_out, "w") as outfile:
        ...
        header_new = header + tsv_header(args.gnomad_type, args.add_allele_count, args.add_subpopulations)
        ...
        for line in mgi_tsv_reader:
            ...
            if mgi_key in gnomad_annotations:
                ...
                new_line = line.copy()
                for index, elem in enumerate(trie_header(args.gnomad_type, args.add_subpopulations)):
